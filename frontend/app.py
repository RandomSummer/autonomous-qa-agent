...
def main():
    ...
    st.markdown("""
    AI-powered test case and Selenium script generation from documentation.
    
    **Workflow:** Upload Documents → Build Knowledge Base → Generate Test Cases → Generate Scripts
    """)
    ...
    if health:
        ...
        with col1:
            st.metric("API Status", "🟢 Healthy" if health['status'] == 'healthy' else "🔴 Down")
        with col2:
            st.metric("Groq API", "✅ Configured" if health['groq_api_configured'] else "❌ Missing")
        with col3:
            st.metric("Vector DB", "✅ Ready" if health['vector_db_initialized'] else "⚠️ Empty")
    
    # Sidebar for navigation
    st.sidebar.title("📋 Navigation")
    page = st.sidebar.radio(
        "Choose a step:",
        ["1️⃣ Upload Documents", "2️⃣ Build Knowledge Base", "3️⃣ Generate Test Cases", "4️⃣ Generate Scripts"]
    )
    ...
    if page == "1️⃣ Upload Documents":
        ...
        # Clear uploads
        if st.button("🗑️ Clear All Uploads", help="Delete all uploaded files"):
    ...
    elif page == "2️⃣ Build Knowledge Base":
    ...
    elif page == "3️⃣ Generate Test Cases":
        ...
        **How it works:**
        ...
        if submitted and query:
            with st.spinner("Generating test cases... Using RAG to retrieve context..."):
                ...
                if result:
                    if result['success']:
                        st.success(f"✅ Generated {result['total_cases']} test cases!")
                        ...
                        for i, tc in enumerate(result['test_cases']):
                            with st.expander(f"**{tc['test_id']}: {tc['feature']}** ({tc['test_type']})", expanded=i==0):
                                ...
                                st.info(f"📚 **Grounded in:** {tc['grounded_in']}")
                                
                                # Copy as JSON
                                if st.button(f"📋 Copy JSON", key=f"copy_{tc['test_id']}"):
    ...
    # ========================================================================
    ...
    elif page == "4️⃣ Generate Scripts":
        ...
        **Requirements:**
        ...
        if 'test_cases' not in st.session_state or not st.session_state['test_cases']:
            st.warning("⚠️ No test cases found. Please generate test cases first (Step 3).")
            st.info("👈 Go to **3️⃣ Generate Test Cases** to create test cases.")
        else:
            test_cases = st.session_state['test_cases']
            st.success(f"✅ {len(test_cases)} test cases available")
            ...
            if html_path.exists():
                with open(html_path, 'r', encoding='utf-8') as f:
                    html_content = f.read()
                st.success(f"✅ HTML loaded: {len(html_content)} characters")
            else:
                st.warning("⚠️ No HTML file found. Please upload checkout.html in Step 1.")
